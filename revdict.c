...
void *revdctget(Dict *dct, int key) {
...
void revdctinsert(Dict *dct, int key, void *val) {
        //int loadFactor = 0;
        unsigned long int idx = 0;
        Node *curr, *node;
        if (dct == NULL || key == -1) {
...
void *revdctremove(Dict *dct, char *key) {
        unsigned long int idx = 0;
        int *val = NULL;
        Node *prev = NULL;
        Node *curr = NULL;
